...
interface ActivateBody {
  // Acepta el código corto ("PET-000123", para tipear a mano) o el token
  // opaco largo (el que va en la URL del QR) — lo que la persona tenga a
  // mano. El QR físico solo trae el token; el código corto es el respaldo
  // para cuando escribir a mano es más fácil que escanear.
  code: string;
  pet: {
    name: string;
...
export async function POST(request: NextRequest) {
  ...
  if (!body?.code || !body?.pet?.name || !body?.pet?.species) {
